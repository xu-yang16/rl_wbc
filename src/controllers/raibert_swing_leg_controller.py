...
def compute_desired_foot_positions(
    ...
    rl_action_residuals,  # Added by yx, tensor of shape (num_envs, 3)
):
    ...
    mid_position[..., 2] = (
        -base_height[:, None] + foot_height + rl_action_residuals[:, 2, None]
    )
    ...
    residuals = torch.zeros_like(base_velocity)  # (num_envs, 6)
    residuals[:, :2] = rl_action_residuals[:, :2]

    land_position = (
        hip_velocity * stance_duration[:, :, None] / 2 + residuals[:, None, :]
    )
    land_position[..., 0] = torch.clip(land_position[..., 0], -0.2, 0.2)
    land_position[..., 1] = torch.clip(land_position[..., 1], -0.11, 0.11)
    ...
    foot_position = _gen_swing_foot_trajectory(
        normalized_phase, phase_switch_foot_positions, mid_position, land_position
    )  # normalized_phase: (num_envs, 1), phase_switch_foot_positions: (num_envs, 4, 3), mid_position: (num_envs, 4, 3), land_position: (num_envs, 4, 3)
...
class RaibertSwingLegController:
    ...
    def __init__(
        ...
        foot_landing_clearance: float = -0.0,
    ...
    def reset(self) -> None:
        ...
        self._phase_switch_foot_positions = torch.matmul(
            ...
            self._robot.foot_positions_in_base_frame.transpose(1, 2),
        ).transpose(1, 2)
    ...
    def reset_idx(self, env_ids) -> None:
        ...
        self._phase_switch_foot_positions[env_ids] = torch.matmul(
            ...
            self._robot.foot_positions_in_base_frame[env_ids].transpose(1, 2),
        ).transpose(1, 2)
    ...
    def desired_foot_positions(self):
        ...
        """
        temp = compute_desired_foot_positions(
            self._robot.base_rot_mat,
            ...
            self._phase_switch_foot_positions,
            torch.zeros(self._num_envs, 12, device=self._device),
        )
        return temp

    def with_residuals_desired_foot_positions(self, rl_action_residuals):
