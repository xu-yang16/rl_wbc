...
class MotorGroup:
    ...
    def __init__(
        ...
        self._device = device
        self._init_positions = to_torch(
            [motor._init_position for motor in motors], device=device
        )
        self._min_positions = to_torch(
            [motor._min_position for motor in motors], device=device
        )
        self._max_positions = to_torch(
            [motor._max_position for motor in motors], device=device
        )
        self._min_velocities = to_torch(
            [motor._min_velocity for motor in motors], device=device
        )
        self._max_velocities = to_torch(
            [motor._max_velocity for motor in motors], device=device
        )
        self._min_torques = to_torch(
            [motor._min_torque for motor in motors], device=device
        )
        self._max_torques = to_torch(
            [motor._max_torque for motor in motors], device=device
        )
    ...
    def convert_to_torque(
        ...
        if motor_control_mode == MotorControlMode.POSITION:
            ...
            self._torque_output = (
                0 * self._torque_output + 1.0 * self._torque_history[0]
            )
