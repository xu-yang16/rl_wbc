...
class RobotStateEstimator:
    ...
    def __init__(
        ...
        sensor_variance: np.ndarray = np.array(
            [0.33705298, 0.14858707, 0.68439632, 0.68]
        )
        * 0.03,
        ...
        ic(self.robot.control_timestep)
        ...
        self.filter.x = np.array([0.0, 0.0, 0.0, 0.26])
        ...
        self._estimated_position = np.array([0.0, 0.0, self.robot.mpc_body_height])
        self.reset()
    ...
    def update(self, robot_state):
        ...
        calibrated_acc[:3] = rot_mat.dot(sensor_acc) + np.array([0.0, 0.0, -9.8])
        calibrated_acc[3] = self._estimated_velocity[2]
        self.filter.predict(u=calibrated_acc * delta_time_s)

        (observed_velocities, observed_heights) = (
            self._get_velocity_and_height_observation()
        )
    ...
    def local_estimated_velocity(self):
        return self.robot._base_rot_mat.T.dot(self._estimated_velocity)
